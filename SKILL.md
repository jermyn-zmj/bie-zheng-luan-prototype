...
description: "将产品原型转换为详细技术规范的技能。当产品经理提供原型资源需要拆解为技术实现方案时使用。支持四种输入类型：1.URL原型（Figma/墨刀/Axure等公开链接）→ 下载HTML并解析DOM结构；2.本地HTML文件 → 直接读取并解析；3.图片原型（PNG/JPG/WebP等设计稿截图）→ 提取颜色、布局区域，建议配合视觉模型增强分析；4.XMind文件（产品脑图/信息架构）→ 解析zip结构，提取节点层级，推断功能模块、页面、功能点。输出包含前端组件描述、后端接口设计、数据库表设计的完整技术文档，供大模型生成90%相似度系统。"
version: "2.3.0"
---
